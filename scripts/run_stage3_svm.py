"""
Stage 3 - SVM training and kernel optimization.

Chạy file này để hoàn thành phần việc Stage 3:
...
def _build_report_markdown(summary: dict, kernel_plot: Path, cm_plot: Path) -> str:
    ...
    return f"""# Progress Report 3 - SVM Training and Evaluation

## 1. Nhiệm vụ thực hiện

Trong Giai đoạn 3, phần việc này là huấn luyện và tối ưu mô hình SVM trên dữ liệu đã tiền xử lý. Mục tiêu là thử nghiệm nhiều kernel khác nhau, so sánh hiệu quả theo cross-validation, sau đó đánh giá mô hình tốt nhất trên tập test.
...
Phần việc Stage 3 đã hoàn thành: huấn luyện SVM, thử nhiều kernel, chọn cấu hình tốt nhất và tạo đầy đủ artifact phục vụ báo cáo.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Stage 3 SVM workflow")
    parser.add_argument(
        "--max-train-samples",
        type=int,
        default=5000,
    ...
    print("[Stage3] Load dataset...")
    df = load_dataset_from_config()

    print("[Stage3] Preprocess với train/test split...")
    ...
    if sample_cap > 0 and len(X_train) > sample_cap:
        ...
        print(
            "[Stage3] Downsample train set cho GridSearchCV: "
            f"{X_train.shape[0]} samples"
        )

    print("[Stage3] Tối ưu SVM qua nhiều kernel...")
    ...
    if args.quick:
        ...
        print(
            "[Stage3] Quick tuning enabled: thử 3 kernel với grid gọn "
    ...
    report_path = output_dir / "svm_stage3_progress_report.md"
